module EventsListHelper
  ...
  def recent_events_list(limit = Spree::Config[:event_list_recent])
    ...
    Event.publish.find(:all, :limit => limit).collect { |event| recent_event.new(event.title, "/events/show/#{event.id}", event.begin_event, event.end_event, event.venue, event.summary) }
  end
